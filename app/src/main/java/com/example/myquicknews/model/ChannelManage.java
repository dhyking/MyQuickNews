...
import com.example.myquicknews.db.ChannelDao;
import com.example.myquicknews.db.SQLHelper;
...
public class ChannelManage {
	...
	public List<ChannelItem> getUserChannel() {
		...
		if (cacheList != null && !((List) cacheList).isEmpty()) {
			...
			for (int i = 0; i < count; i++) {
				...
				navigate.setSelectedState(Integer.valueOf(maplist.get(i).get(SQLHelper.SELECTED)));
				list.add(navigate);
			}
			return list;
		}
	...
	public void saveUserChannel(List<ChannelItem> userList) {
		for (int i = 0; i < userList.size(); i++) {
			...
			channelItem.setSelectedState(Integer.valueOf(1));
	...
	public void saveOtherChannel(List<ChannelItem> otherList) {
		for (int i = 0; i < otherList.size(); i++) {
			...
			channelItem.setOrderId(i);
			channelItem.setSelectedState(Integer.valueOf(0));
			channelDao.addCache(channelItem);
		}
	}
